...
-   Python 3.8+ (`python3`, `python3-dev`, `python3-pip`)
-   Git
-   **7-Zip** (`p7zip-full` on Ubuntu, `p7zip` on macOS via Homebrew) - **Critical for packaging**
-   Make
...
sudo apt-get install -y build-essential python3 python3-dev python3-pip \
    git p7zip-full golang-go msitools wget aria2 curl rsync rustc

# macOS (install Homebrew first: https://brew.sh)
brew install python3 p7zip go wget aria2 git make curl rsync
curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh
xcode-select --install

# Fedora/RHEL
sudo dnf install -y python3 python3-devel p7zip golang msitools wget aria2 \
    git make curl rsync
```

**Optional but recommended: Install sccache for faster builds**

[sccache](https://github.com/mozilla/sccache) is a compiler cache that can dramatically reduce rebuild times (50-90% faster after the initial build). Install it before building:
...
After installing sccache, add these lines to `assets/base.mozconfig`:

```bash
# Enable sccache for faster builds
ac_add_options --with-ccache=sccache

# Optional: Configure sccache cache directory and size
export SCCACHE_DIR="$HOME/.cache/sccache"
export SCCACHE_CACHE_SIZE="50G"
```

To verify sccache is working after your first build:

```bash
sccache --show-stats
```

**Note:** The first build will be slower as it populates the cache. Subsequent builds will be significantly faster.
