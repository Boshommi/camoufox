...
**`/gsd:auto-execute <phase>`**
Autonomously plan and execute all plans in a phase.

- Uses subagents for fresh context on each plan
- One subagent for planning (if no plans exist)
- One subagent per plan execution (sequential)
- Batches verify checkpoints at end of phase
- Pauses only for decision checkpoints
- Main context only orchestrates (~10-15% usage)

Usage: `/gsd:auto-execute 14`
...
**Executing an entire phase autonomously:**

```
/gsd:auto-execute 5  # Plans (if needed) and executes all plans in phase 5
```
