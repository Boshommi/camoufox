/**
 * Bundled Fingerprint Detector for Camoufox
 * Auto-generated from services/FE/src/lib/fingerprint-detector.ts
 *
 * Usage: Inject this script via page.evaluate() and call detectAll()
 */
...
(function() {
  ...
  function detectNavigator() {
    ...
    config["navigator.vendor"] = safeGet(() => navigator.vendor);
    config["navigator.vendorSub"] = safeGet(() => navigator.vendorSub);

    // Array properties
    config["navigator.languages"] = safeGet(() => Array.from(navigator.languages));

    // Numeric properties
    config["navigator.hardwareConcurrency"] = safeGet(() => navigator.hardwareConcurrency);
    ...
    config["navigator.globalPrivacyControl"] = safeGet(() => navigator.globalPrivacyControl);
  ...
  function detectAudioContext() {
    ...
    try {
      ...
      if ("outputLatency" in ctx) {
        config["AudioContext:outputLatency"] = ctx.outputLatency;
      }
  ...
  /**
   * Get precision name from GL constant
   */
  function getPrecisionName(gl, precision) {
    switch (precision) {
      case gl.LOW_FLOAT:
        return "lowFloat";
      ...
      for (const precisionType of precisionTypes) {
        try {
          ...
          if (format) {
            ...
            formats[shaderKey][precisionKey] = {
  ...
  function detectWebGL() {
    ...
    if (gl) {
      ...
      if (debugInfo) {
        config["webGl:renderer"] = gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL);
   ...
   */
  async function detectUAClientHints() {
    const config = {};
